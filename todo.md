...
- [ ] **WebRTC desktop streaming (2026)** — implementation landed; runtime validation on full GPU stack pending
  ...
  - [ ] **Microphone input (browser → desktop)**: audio transceiver `sendrecv` + `getUserMedia` with explicit consent UX (off by default, session-policy flag); virtual mic source in `pulse-default.pa` (`module-pipe-source` or `axonos_mic` null sink set as default source); agent decodes the inbound track and writes into it — mirror image of `open_audio_capture`; needs the same teardown discipline as the input data channels
